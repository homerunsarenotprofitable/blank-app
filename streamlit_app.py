...
def kelly_fraction(prob, payout):
    ...
    return max(f, 0)  # no negative bet
...
with tab1:
    ...
    if not ev.empty:
        ...
        st.dataframe(top10[display_cols].style.applymap(ev_bar, subset=["EV"]), use_container_width=True)
    else:
...
with tab2:
    ...
    if not ev.empty:
        ...
        slate_payout = st.number_input("Enter slate payout multiplier", value=2.0, min_value=1.0, step=0.1)

        if selections:
            selected_rows = ev[ev["player"] + " | " + ev["prop"].isin(selections)]
            probs = selected_rows["baseline_prob"].tolist()
            combined_prob = 1
            for p in probs:
                combined_prob *= p
            slate_ev = combined_prob * slate_payout - (1 - combined_prob)
            ...
            st.metric("Suggested Units", f"${round(suggested_units*100,2)}")
        else:
...
with tab3:
    ...
    if not ev.empty:
        ...
        slate_payout = st.number_input("Enter slate payout multiplier for suggestions", value=2.0, min_value=1.0, step=0.1, key="suggestion_payout")

        slates = []
        for r in range(2, max_slate_size + 1):
            for combo in combinations(top_picks["player"] + " | " + top_picks["prop"], r):
                probs = [top_picks[top_picks["player"] + " | " + top_picks["prop"] == c]["baseline_prob"].values[0] for c in combo]
                combined_prob = 1
                for p in probs:
                    combined_prob *= p
                slate_ev = combined_prob * slate_payout - (1 - combined_prob)
                ...
                slates.append({
                    ...
                    "Suggested Units": round(suggested_units,2)
                })
